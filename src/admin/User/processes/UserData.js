...
var UserData = function(args){
    ...
    this.getRestaurant = function(data,user_data,next){
        ...
        data.forEach(function(element){
            Business.findById({merchant_id:element.merchant_id , business_id:element.business_id},function(err,result){
                if(err) return next(err);
                if(result.Business.status == "active")
                    res.push(result);
                count++;
    ...
    this.getRestaurantDetails = function(data,user_data,next){
        ...
        data.forEach(function(element){
            var value = {
                merchant_id : element.Business.merchant_id,
                business_id : element.Business.business_id
            }
            Restaurant.findRestaurant(value,function(err,result){
                if(err) return next(err);
                result[0]['name'] = element.Business.name;
                result[0]['address'] = element.Business.address;
                result[0]['gstin'] = element.Business.gstin;
                result[0]['open'] = true;
